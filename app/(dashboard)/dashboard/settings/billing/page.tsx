...
export default function BillingSettingsPage() {
      ...
      for (const unitFee of unitFees) {
        ...
        if (amount > 0) {
          items.push({
            ...
            amount,
            meter_reading_id: (items[items.length - 1] as any)
              ?.meter_reading_id, // Fix: ensure we don't drop fields if pushing differently
          });
          // Wait, the push block was duplicated in original code?
          // Let's fix the structure. In original code:
          /*
            if (amount > 0) {
              items.push({
                fee_type_id: feeType.id,
          ...
          */
          // The metered case has its own push.
        }
      }
  ...
  if (success) {
    return (
      <>
        <Header title="Төлбөрийн тохиргоо" showBack />
        <div className="p-6">
